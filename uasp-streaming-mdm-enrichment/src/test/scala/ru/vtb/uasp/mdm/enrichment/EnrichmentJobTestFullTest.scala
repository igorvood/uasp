...
import ru.vtb.uasp.mdm.enrichment.service.dto.NotStandardDataStreams
...
class EnrichmentJobTestFullTest extends AnyFlatSpec with Matchers {
  ...
  it should "send all type msg" in {
    ...
    val streams = NotStandardDataStreams(mainStream, Some(hypothecInputDataStream), Some(globalDataStream))
  ...
  it should " fill error if not found global id" in {
    ...
    val streams = NotStandardDataStreams(mainStream, Some(emptyStream), Some(emptyStream))
