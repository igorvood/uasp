...
class KafkaRequestActionBuilder[K,V](kafkaAttributes: KafkaAttributes[K,V]) extends ActionBuilder {
  ...
  private[this] var checkRequestFunc: Option[(Session,Option[Throwable]) => Unit] = None
  ...
  def checkRequest(callback: (Session,Option[Throwable]) => Unit): KafkaRequestActionBuilder[K,V] = {
    checkRequestFunc = Some(callback)
    this
  }
