...
class TestDataDtoMaskedSerializeService(jsMaskedPath: Option[JsMaskedPath]) extends AbstractOutDtoWithErrorsMaskedSerializeService[TestDataDto](jsMaskedPath) {
  override def convert(value: OutDtoWithErrors[TestDataDto], jsMaskedPath: Option[JsMaskedPath]): Either[List[JsMaskedPathError], KafkaDto] = value.serializeToBytes(jsMaskedPath)
}
