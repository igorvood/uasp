...
case class NewOutDtoWithErrors[T](
...
object NewOutDtoWithErrors {

  implicit def outDtoWithErrorsJsonReads[T](implicit fmt: Reads[T]): Reads[NewOutDtoWithErrors[T]] = json =>
    for {
    ...
    } yield new NewOutDtoWithErrors[T](serviceDataDto, errorPosition, errors, data)


  implicit def outDtoWithErrorsJsonWrites[T](implicit fmt: OWrites[T]): OWrites[NewOutDtoWithErrors[T]] = {
    ...
    value(unlift(NewOutDtoWithErrors.unapply[T]))
  }
