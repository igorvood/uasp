...
import org.apache.flink.streaming.api.operators.{KeyedProcessOperator, OneInputStreamOperator, StreamMap}
...
class AggregateFirstSalaryRichMapFunctionTest extends AnyFlatSpec {
  ...
  it should " test enrichMainStream with Optional value but it is empty" in {
    ...
    val aggregateFirstSalaryRichMapFunction = new AggregateFirstSalaryRichMapFunction(ConfirmedPropsModel(stringToString))
